...
app.get('/minerdata', function(req, res) {
  async function mphfunction() {
    const mphurl =
      ...
      keys.mphKey;
    const mph = await axios
      .get(mphurl, { responseType: 'application/json' })
  ...
  async function secondtry(val) {
    ...
    const mktcap = await axios
      .get('https://api.coinmarketcap.com/v1/ticker/' + val.coin + '/', {
  ...
  async function mktGet(array) {
    ...
    for (const item of array) {
      await secondtry(item).then(mktcap => {
        arrayCoin.push(mktcap);
      });
    }
    res.send({ data: arrayCoin });
  }

  mphfunction().then(mph => {
    mktGet(mph);
  });
});
